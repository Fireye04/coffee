...
async function newsletterSubmit() {
	const Email = document
		.getElementsByClassName("nsEmail")[0]
		.value.toString()
		.trim();
	...
	if (document.getElementsByClassName("nsMusic")[0].checked) {
		Newsletters.push("Music");
	}
	document.getElementsByClassName("nsMusic")[0].checked = false;
	...
	try {
		const response = await fetch(
			"https://coffee-newsletter.codekai16.workers.dev/api",
			{
				headers: {
